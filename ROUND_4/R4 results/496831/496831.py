...
# Product-specific ladder controls (for T1/T2 isolation).
HP_L2_RATIO = 0.45
VEX_L2_RATIO = 0.35
VEV_L2_RATIO = 0.15
...
class Trader:
    ...
    def _mm_orders(
        ...
        if l2_ratio > 0:
            # Second passive level: less aggressive to capture slower flow.
            q2 = max(1, int(round(q * float(l2_ratio))))
    ...
    def run(self, state: TradingState):
        ...
        for sym, base_qty in ((HP, 20), (VEX, 18)):
            ...
            orders = self._mm_orders(
                ...
                base_qty=base_qty,
                cp_sig=cp_sig,
                l2_ratio=(HP_L2_RATIO if sym == HP else VEX_L2_RATIO),
            )
            if orders:
                result[sym] = orders
