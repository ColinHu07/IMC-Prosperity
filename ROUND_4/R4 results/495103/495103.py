...
# Method C: product-role split parameters for VEV_4000.
VEV4000_NEUTRAL_BAND = 40
VEV4000_STRETCHED_BAND = 80
VEV4000_MIN_SPREAD = 18.0
VEV4000_CP_FLOOR = 0.0
    ...
    def run(self, state: TradingState):
        ...
        for sym in PASSIVE_VOUCHERS:
            ...
            if cp_sig > 20:
                base += 1
            if sym == VEV4000:
                spread_now = float(min(asks) - max(bids)) if (bids and asks) else 0.0
                # Soft degradations for weak setup; avoid hard skip.
                if spread_now < VEV4000_MIN_SPREAD:
                    base = max(1, int(base * 0.7))
                if cp_sig < VEV4000_CP_FLOOR:
                    base = max(1, int(base * 0.7))

                # Inventory-band policy:
                # neutral: two-sided; stretched: one-sided flattening;
                # extreme: flatten aggressively toward stretched band.
                abs_p = abs(p)
                if abs_p > VEV4000_STRETCHED_BAND:
                    orders: List[Order] = []
                    target_flat = abs_p - VEV4000_NEUTRAL_BAND
                    clip = min(25, max(1, target_flat))
                    if p > 0 and bids:
                        orders.append(Order(sym, int(max(bids)), -int(min(clip, p))))
                    elif p < 0 and asks:
                        orders.append(Order(sym, int(min(asks)), int(min(clip, -p))))
                    if orders:
                        result[sym] = orders
                    continue
            fair = mp
            ...
            if sym == VEV4000 and orders:
                if p > VEV4000_NEUTRAL_BAND:
                    # Stretched long inventory: only keep sell quotes.
                    orders = [o for o in orders if int(o.quantity) < 0]
                elif p < -VEV4000_NEUTRAL_BAND:
                    # Stretched short inventory: only keep buy quotes.
                    orders = [o for o in orders if int(o.quantity) > 0]
            if orders:
                result[sym] = orders
