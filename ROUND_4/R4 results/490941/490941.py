...
try:
    ...
    try:
        ...
        from ROUND_3.datamodel import Order, OrderDepth, TradingState
...
class Trader:
    ...
    def _mm_orders(
        ...
        if buy_room > 0:
            orders.append(Order(sym, bid_px, min(q, buy_room)))
        if sell_room > 0:
            orders.append(Order(sym, ask_px, -min(q, sell_room)))
        return orders
    ...
    def run(self, state: TradingState):
        ...
        # Preserve simple own-trade realized tracker if available.
        # Backtest harness may not pass own_trades, so this defaults to 0.
        ...
        # ---------- Core books: HP + VEX ----------
        for sym, base_qty in ((HP, 18), (VEX, 16)):
        ...
        for sym in PASSIVE_VOUCHERS:
            ...
            if voucher_frozen:
                ...
                orders: List[Order] = []
                if p > 0 and bids:
                    orders.append(Order(sym, max(bids), -min(p, 20)))
                elif p < 0 and asks:
                    orders.append(Order(sym, min(asks), min(-p, 20)))
                if orders:
                    result[sym] = orders
                continue
        ...
        for sym in (VEV4500, VEV5000, VEV5100, VEV6000, VEV6500):
            ...
            bids, asks = _book(od)
            if p > 0 and bids:
                result[sym] = [Order(sym, max(bids), -min(20, p))]
            elif p < 0 and asks:
                result[sym] = [Order(sym, min(asks), min(20, -p))]
