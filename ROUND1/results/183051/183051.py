...
class Trader:
    """
    HYPOTHESIS: Ash fair value is essentially constant at 10000.
    CHANGE: fair = 10000 always (EWMA still computed but not used for fair).
    RATIONALE: If the fair value really is 10000 +/- noise, a hard anchor has
    zero lag and maximizes the take-or-make edge. Risk: if fair ever moves
    away from 10000, this becomes systematically wrong.
    Pepper: UNCHANGED.
    """
    ...
    def _trade_ash(self, sym, bids, asks, mid, position, old):
        ...
        ewma = old.get("a_ewma")
        if ewma is None:
            ewma = mprice if mprice is not None else 10000.0
        elif mprice is not None:
            ewma = EWMA_ALPHA * mprice + (1 - EWMA_ALPHA) * ewma

        fair = 1.0 * ANCHOR + 0.0 * ewma
        td = {"a_ewma": ewma}
        orders = []
        if fair is None:
            return orders, td
