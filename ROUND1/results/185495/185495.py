...
class Trader:
    ...
    def _trade_pepper(self, sym, bids, asks, mid, position, old):
        ...
        # Build long exposure with controlled aggression:
        # lift only near-touch asks to avoid paying through the full stack.
        if asks:
            sorted_asks = sorted(asks)
            room = LIMIT - pos
            if room > 0:
                best_ask = sorted_asks[0]
                max_premium = 4 if pos < LIMIT * 0.5 else 2
                if best_ask <= fair + max_premium:
                    vol = min(asks[best_ask], MAX_SIZE, room)
                    if vol > 0:
