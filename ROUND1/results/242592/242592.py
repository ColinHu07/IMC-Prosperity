...
ASH_PASSIVE_SIZE = 15          # Nominal passive size (used when spread ~ typical)
...
ASH_TYPICAL_SPREAD = 4.0       # Reference spread for size scaling (observed ~4 at top-of-book)
ASH_PASSIVE_SIZE_MIN = 8
ASH_PASSIVE_SIZE_MAX = 25
...
class Trader:
    ...
    def _trade_ash(self, sym, bids, asks, mid, position, old):
        ...
        # Spread-conditional passive size: wider spread = more profit per fill
        # -> worth quoting more size.  Tight spread = mostly noise, quote less.
        spread = 0.0
        if bids and asks:
            spread = max(0.0, min(asks) - max(bids))
        _scaled = ASH_PASSIVE_SIZE * (spread / ASH_TYPICAL_SPREAD) if spread > 0 else ASH_PASSIVE_SIZE
        PASSIVE_SIZE = int(max(ASH_PASSIVE_SIZE_MIN, min(ASH_PASSIVE_SIZE_MAX, _scaled)))
